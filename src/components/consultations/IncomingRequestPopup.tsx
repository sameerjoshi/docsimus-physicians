...
import { ConsultationRequestEvent } from '@/src/types/consultations';
import {
  Phone,
  X,
  User,
            ...
            <div className="bg-accent/50 rounded-lg p-3 mb-4 space-y-2">
              {data.reason && (
                <div className="flex items-start gap-2 text-sm">
                  ...
                  <span className="line-clamp-2">{data.reason}</span>
                </div>
              )}
              {data.symptoms && (
                <div className="flex items-start gap-2 text-sm">
                  <Stethoscope className="h-4 w-4 text-muted-foreground mt-0.5" />
                  <span className="line-clamp-2">{data.symptoms}</span>
                </div>
              )}
            </div>
          )}
